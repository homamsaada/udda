...
ملف `session-todos.md` (على جذر المشروع، gitignored) هو **مرآة حيّة (live mirror)** لقائمة TodoWrite الخاصة بك، يراها المستخدم كلوحة تقدّم بصرية أثناء عملك.
...
### الإنشاء الأوّلي (عند بداية الجلسة)

عند بداية أي جلسة تتطلّب TodoWrite، استخدم **`Write`** على `session-todos.md` بحالته الأوّلية (ينشئ لو غير موجود، يستبدل المحتوى القديم لو كان من جلسة سابقة على نفس الـ worktree). كل التحديثات بعد ذلك بالـ `Edit`.

**استثناء وحيد لقاعدة "لا تُعد كتابة الملف من الصفر".** `session-todos.md` عابر per-session؛ محتوى الجلسة السابقة لا علاقة له بالحالية. كل جلسة تبدأ بلوحة نظيفة.
