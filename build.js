...
function buildHomepage(lang) {
  ...
  const categoriesHTML = toolsData.categoryOrder.map(catId => {
    ...
    const catTools = toolsData.tools.filter(t => t.category === catId);
    if (catTools.length === 0) return '';
    
    const toolsHTML = catTools.map(tool => {
      const toolI18n = i18n.tools[tool.id]?.[lang];
      return `
            <a href="/${lang}/tools/${tool.id}.html" class="category-tool-item">
              <span class="category-tool-icon">${tool.icon}</span>
              <span class="category-tool-name">${toolI18n?.name || tool.id}</span>
            </a>`;
    }).join('');
    ...
    return `
        <div class="category-accordion">
          <div class="category-accordion-header" onclick="toggleCategory(this)">
            <div class="category-accordion-info">
              <span class="category-accordion-icon">${cat.icon}</span>
              <span class="category-accordion-name">${cat.name}</span>
              <span class="category-accordion-count">(${catTools.length})</span>
            </div>
  ...
  const content = `
    ...
    <script>
      ...
      function toggleCategory(header) {
        ...
        // Close all others
        document.querySelectorAll('.category-accordion.open').forEach(el => {
